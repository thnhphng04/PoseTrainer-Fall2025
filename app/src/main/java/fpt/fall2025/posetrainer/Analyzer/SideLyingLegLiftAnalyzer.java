...
/**
 * SideLyingLegLift Analyzer - Phân tích bài tập SideLyingLegLift
 * Implement ExerciseAnalyzerInterface để có thể sử dụng chung CameraFragment
 */
public class SideLyingLegLiftAnalyzer implements ExerciseAnalyzerInterface {

    private SideLyingLegLiftThresholds thresholds;
    ...
    public SideLyingLegLiftAnalyzer() {
        this.thresholds = SideLyingLegLiftThresholds.defaultBeginner();
    ...
    public SideLyingLegLiftAnalyzer(SideLyingLegLiftThresholds thresholds) {
        this();
    ...
    public ExerciseFeedback analyze(List<Map<String, Float>> landmarks) {
        ...
        cameraWarning = offsetAngle < 60 || positionCheck < 60;
        ...
        if (cameraWarning) {
            ...
            // Tính các góc mới
            int thighSeparationAngle = calculateAngle(leftKnee, leftHip, rightHip, rightKnee);
    ...
    public Map<String, Object> getThresholds(String level) {
        ...
        if ("pro".equals(level)) {
            SideLyingLegLiftThresholds proThresholds = SideLyingLegLiftThresholds.defaultPro();
            result.put("separationThreshold", proThresholds.getSeparationThreshold());
        } else {
            result.put("separationThreshold", thresholds.getSeparationThreshold());
        }
        return result;
    }
    ...
    public void updateThresholds(Map<String, Object> thresholds) {
        if (thresholds.containsKey("legNormal")) {
            this.thresholds.setSeparationThreshold((int[]) thresholds.get("separationThreshold"));
        }
    }
    ...
    private int calculateAngle(Map<String, Float> p1, Map<String, Float> p2, Map<String, Float> p3, Map<String, Float> p4) {
        if (p1 == null || p2 == null || p3 == null || p4 == null) return 0;

        float[] a = {p1.get("x") - p2.get("x"), p1.get("y") - p2.get("y")};
        float[] b = {p4.get("x") - p3.get("x"), p4.get("y") - p3.get("y")};
        float dot = a[0] * b[0] + a[1] * b[1];
        float normA = (float) Math.sqrt(a[0] * a[0] + a[1] * a[1]);
        float normB = (float) Math.sqrt(b[0] * b[0] + b[1] * b[1]);
        float cosTheta = Math.max(-1f, Math.min(1f, dot / (normA * normB)));
    ...
    private String getState(int thighSeparationAngle) {
        if (thighSeparationAngle < thresholds.getSeparationThreshold()[0]) {
            return "s1";
        } else if (thighSeparationAngle > thresholds.getSeparationThreshold()[1]) {
            return "s2";
        }
        return null;
    }
    ...
    // Inner class for SideLyingLegLiftThresholds
    public static class SideLyingLegLiftThresholds {
        private int[] separationThreshold;
        private int offsetThresh;
        private double inactiveThresh;
        private int cntFrameThresh;
        
        public SideLyingLegLiftThresholds() {}
        
        public SideLyingLegLiftThresholds(int[] separationThreshold,
        ...
        public static SideLyingLegLiftThresholds defaultBeginner() {
            return new SideLyingLegLiftThresholds(
                new int[]{10, 30},
                45, 15.0, 50
            );
        }
        
        public static SideLyingLegLiftThresholds defaultPro() {
            return new SideLyingLegLiftThresholds(
                new int[]{10, 35},
                45, 15.0, 50
            );
