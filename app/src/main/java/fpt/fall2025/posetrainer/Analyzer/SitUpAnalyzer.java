...
/**
 * SitUp Analyzer - Phân tích bài tập Sit-Up
 * Implement ExerciseAnalyzerInterface để có thể sử dụng chung CameraFragment
 */
public class SitUpAnalyzer implements ExerciseAnalyzerInterface {

    private SitUpThresholds thresholds;
    ...
    public SitUpAnalyzer() {
        this.thresholds = SitUpThresholds.defaultBeginner();
    ...
    public SitUpAnalyzer(SitUpThresholds thresholds) {
        this();
    ...
    public ExerciseFeedback analyze(List<Map<String, Float>> landmarks) {
        ...
        if (cameraWarning) {
            ...
            int shoulderAngle = calculateAngle(ear, shldr, hip);   // Góc ear-shoulder-hip
            ...
            if ("s2".equals(currState)) {
                ...
                if (complete) {
                    ...
                    if (kneeAngle < thresholds.getKneeThresholds()[0]) {
                        ...
                        incorrectCount++;
                        message = "INCORRECT";
                    } else if (hipAngle > thresholds.getHipThresholds()[1]) {
                        displayText[2] = true;
                        incorrectPosture = true;
                        feedbackList.add("Nâng người lên cao hơn");
                        incorrectCount++;
                        message = "INCORRECT";
                    }
    ...
    public Map<String, Object> getThresholds(String level) {
        ...
        if ("pro".equals(level)) {
            SitUpThresholds proThresholds = SitUpThresholds.defaultPro();
    ...
    private String getState(int hipAngle) {
        // s1: Nằm xuống (hip angle lớn - thân gần thẳng)
        // s2: Ngồi lên (hip angle nhỏ - thân gập)
        if (hipAngle > thresholds.getHipThresholds()[1]) {
            return "s1";  // Down position (lying down)
        } else if (hipAngle < thresholds.getHipThresholds()[0]) {
            return "s2";  // Up position (sitting up)
        }
        return prevState; // Maintain previous state if in transition
    }
    ...
    // Inner class for SitUpThresholds
    public static class SitUpThresholds {
        private int[] hipThresholds;        // [min, max] - góc hip để phân biệt nằm/ngồi
        private int[] kneeThresholds;       // [min, max] - góc knee phải duy trì
        private int offsetThresh;
        private double inactiveThresh;
        private int cntFrameThresh;

        public SitUpThresholds() {}

        public SitUpThresholds(int[] hipThresholds, int[] kneeThresholds,
