 ...
 */
public class VUpAnalyzer implements ExerciseAnalyzerInterface {

    private VUpThresholds thresholds;
    ...
    public VUpAnalyzer() {
        this.thresholds = VUpThresholds.defaultBeginner();
    ...
    public VUpAnalyzer(VUpThresholds thresholds) {
        this();
    ...
    public ExerciseFeedback analyze(List<Map<String, Float>> landmarks) {
        ...
        int positionCheck = calculateAngleWithUpVertical(leftEar, leftKnee);
        ...
        if (cameraWarning) {
            ...
            float wristX = wrist.get("x");
            float footX = foot.get("x");
            ...
            // State machine
            currState = getState(hipAngle, wristX, footX, footLength);
    ...
    public Map<String, Object> getThresholds(String level) {
        ...
        if ("pro".equals(level)) {
            VUpThresholds proThresholds = VUpThresholds.defaultPro();
    ...
    private String getState(int hipAngle, float wristX, float footX, float footLength) {
        if (hipAngle > thresholds.getHipNormal()) {
            System.out.println("s1");
            return "s1";
        } else if (hipAngle < thresholds.getHipTran() && Math.abs(wristX-footX) < footLength) {
    ...
    // Inner class for VUpThresholds
    public static class VUpThresholds {
