 ...
 */
public class ClapOverHeadAnalyzer implements ExerciseAnalyzerInterface {

    private ClapOverHeadThresholds thresholds;
    ...
    public ClapOverHeadAnalyzer() {
        this.thresholds = ClapOverHeadThresholds.defaultBeginner();
    ...
    public ClapOverHeadAnalyzer(ClapOverHeadThresholds thresholds) {
        this();
    ...
    public ExerciseFeedback analyze(List<Map<String, Float>> landmarks) {
        ...
        if (cameraWarning) {
            ...
            float leftEarX = leftEar.get("x");
            float rightEarX = rightEar.get("x");
            float leftWristX = leftWrist.get("x");
            float rightWristX = rightWrist.get("x");

            Boolean check = leftWristX < leftEarX && rightWristX > rightEarX;


            // State machine
            currState = getState(leftArmAngle, rightArmAngle, check);
            updateStateSequence(currState);


            // Đếm ClapOverHead đúng/sai
            String message = "";
    ...
    public Map<String, Object> getThresholds(String level) {
        ...
        if ("pro".equals(level)) {
            ClapOverHeadThresholds proThresholds = ClapOverHeadThresholds.defaultPro();
    ...
    private String getState(int leftArmAngle, int rightArmAngle, Boolean check) {
        if (leftArmAngle < thresholds.getArmThresholds()[0] && rightArmAngle < thresholds.getArmThresholds()[0]) {
            System.out.println("s1");
            return "s1";
        } else if (leftArmAngle > thresholds.getArmThresholds()[1] && rightArmAngle > thresholds.getArmThresholds()[1] && check) {
    ...
    // Inner class for ClapOverHeadThresholds
    public static class ClapOverHeadThresholds {
        ...
        public ClapOverHeadThresholds() {}

        public ClapOverHeadThresholds(int[] armThresholds,
